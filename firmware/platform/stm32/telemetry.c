...
static void uart_print(const char *format, ...)
{
    ...
    const int length = tiny_vformat(text, sizeof(text), format, args);
    va_end(args);
    /* Formatting overflow fails before TX; never transmit a truncated success. */
